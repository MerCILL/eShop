...
public class CatalogItemRepository : GenericRepository<CatalogItemEntity>, ICatalogItemRepository
{
    ...
    public async Task<CatalogItemEntity> GetByTitleAndPicture(string title, string pictureFile)
    {
        return await _dbContext.Items
            .AsNoTracking()
            .FirstOrDefaultAsync(item => item.Title == title && item.PictureFile == pictureFile);
    }

    public async Task<CatalogTypeEntity> GetTypeByTitle(string title)
    {
        return await _dbContext.Types.FirstOrDefaultAsync(t => t.Title == title);
    }

    public async Task<CatalogBrandEntity> GetBrandByTitle(string title)
    {
        return await _dbContext.Brands.FirstOrDefaultAsync(b => b.Title == title);
    }
    ...
    public override async Task<CatalogItemEntity> Update(CatalogItemEntity entity)
    {
        ...
        await _dbContext.Items
            ...
            .SetProperty(p => p.Quantity, p => entity.Quantity)
    ...
    public async Task<CatalogItemEntity> UpdateQuantity(CatalogItemEntity entity)
    {
        await _dbContext.Items
            .Where(item => item.Id == entity.Id)
            .ExecuteUpdateAsync(sp => sp
